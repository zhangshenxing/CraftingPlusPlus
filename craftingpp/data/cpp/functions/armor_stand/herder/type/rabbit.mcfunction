...
# 堆叠
scoreboard players operation @s cpp_stack += @e[type=rabbit,distance=..16,tag=!cpp_animal_this] cpp_stack
execute as @e[type=rabbit,distance=..16,tag=!cpp_animal_this] run data modify entity @s Health set value 0f
# 繁殖
execute if predicate cpp:golem_herder_rabbit run function cpp:armor_stand/herder/food/rabbit
# 产肉
