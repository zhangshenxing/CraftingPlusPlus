...
# 堆叠
scoreboard players operation @s cpp_stack += @e[type=sheep,distance=..16,tag=!cpp_animal_this,nbt={Color:4b}] cpp_stack
execute as @e[type=sheep,distance=..16,tag=!cpp_animal_this,nbt={Color:4b}] run data modify entity @s Health set value 0f
# 剪羊毛
