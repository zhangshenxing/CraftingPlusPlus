...
execute as @e[type=marker,distance=..7.1,tag=cpp_locate] at @s if block ~ ~ ~ white_wool run tag @s add cpp_locate1
...
execute as @e[type=marker,distance=..7.1,tag=cpp_locate2] at @s if block ~ ~ ~ white_wool run tag @s add cpp_locate1
